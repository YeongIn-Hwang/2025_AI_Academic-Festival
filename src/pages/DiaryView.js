...
export default function DiaryView() {
    ...
    useEffect(() => {
        const fetchDiary = async () => {
            ...
            const trips = snap.data().trips || [];
                 // 같은 도시의 여행만 추리기
                     const sameCity = trips.filter((t) => normalize(t.city) === normalize(region));
                if (sameCity.length === 0) return;

                    // 날짜가 있는 trip을 우선, 없으면 전체 중에서
    ...
    const toggleLike = async (place) => {
        ...
        const tIdx = trips.findIndex((t) => normalize(t.city) === normalize(region));
        if (tIdx === -1) return;
    ...
    return (
        <div className="diaryview-container">
            {/* ✅ 전체 화면 배경 (고정) */}
            <div className="dv-bg" style={{ backgroundImage: `url(${bgFerris})` }} />
            ...
            <h2>📍 {region} 여행 기록</h2>
            {tripDates && (
                       <p className="dv-dates">📅 {tripDates.start} ~ {tripDates.end}</p>
                     )}
                    ...
                    {places.map((p, idx) => (
                        <article key={idx} className="place-card">
                            ...
                            <div className="place-footer">
                                {typeof p.rating === "number" && <StarRating value={p.rating} size={18} />}
                                {p.mapsUrl && (
                                    <a
                                        href={p.mapsUrl}
                                        target="_blank"
